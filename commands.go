...
func handleGeorgeStart(res http.ResponseWriter, req *http.Request) {
	data, err := NewGameStartRequest(req)
	if err != nil {
		respond(res, GameStartResponse{
			Taunt:   toStringPointer("battlesnake-go!"),
...
func handleGeorgeMove(res http.ResponseWriter, req *http.Request) {
	var buffer bytes.Buffer
	start := time.Now()
	data, err := NewGeorgeMoveRequest(req, &buffer)
	log.Printf("Took %s", time.Since(start))
	log.Println()
	if err != nil {
		respond(res, MoveResponse{
